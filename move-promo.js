...
// 1. Extract the Stats Bar
const statsBarRegex = /\{\/\* Stats Bar \*\/}.*?<\/section>/s;

// 2. Updated Promo Section Replacement Code
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-center">
            ...
            <div className="w-full lg:w-[400px]">
              <div className="p-8 bg-zinc-950 border border-zinc-800 rounded-3xl shadow-2xl relative overflow-hidden">
                ...
                <Link href="/pricing" className="flex items-center justify-center w-full bg-yellow-500 hover:bg-yellow-400 text-black py-4 rounded-xl font-black text-lg transition-all active:scale-[0.98] shadow-lg shadow-yellow-500/20">
                  Перейти к тарифам
                </Link>
...
// Replace Stats Bar with the new Promo Section
c = c.replace(statsBarRegex, promoSectionCode);

// 3. Remove the OLD Promo Section at the bottom of the file
const oldPromoRegex = /\{\/\* Promo Section \*\/}.*?\{\/\* Pricing Section \*\/\}/s;
c = c.replace(oldPromoRegex, '{/* Pricing Section */}');

fs.writeFileSync(file, c);
console.log("Promo sector rebuilt and moved successfully.");
